            ...
            <form method="POST" action="{{ route("admin.loans.store") }}" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('individual') ? 'is-invalid' : '' }}"
                        ...
                        @foreach($individuals as $id => $entry)
                            <option
                                value="{{ $id }}" {{ old('individual_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('amount') ? 'is-invalid' : '' }}" type="number"
                           name="amount" id="amount" value="{{ old('amount', '') }}" step="1" required>
                ...
                <div class="form-group">
                    ...
                    <input class="form-control date {{ $errors->has('done_at') ? 'is-invalid' : '' }}" type="text"
                           name="done_at" id="done_at" value="{{ old('done_at') }}" required>
