...
@section('content')
    <div class="card">
        <div class="card-header">
            {{ trans('global.create') }} {{ trans('cruds.goat.title_singular') }}
        </div>

        <div class="card-body">
            <form method="POST" action="{{ route('admin.goats.store') }}" enctype="multipart/form-data">
                @csrf
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('number_of_goats') ? 'is-invalid' : '' }}" type="number"
                        name="number_of_goats" id="number_of_goats" value="{{ old('number_of_goats', '') }}"
                        step="1">
