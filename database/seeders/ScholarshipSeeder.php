        ...
        while (($data = fgetcsv($file)) !== false) {
            ...
            try {
                ...
                Scholarship::create([
                    'uuid' => Str::uuid(),
                    'project_id' => 2,
                    'names' => mb_trim($data[0] ?? ''),
                    'gender' => mb_trim($data[1] ?? ''),
                    'id_number' => mb_trim($data[2] ?? ''),
                    'district' => mb_trim($data[3] ?? ''),
                    'sector' => mb_trim($data[4] ?? ''),
                    'cell' => mb_trim($data[5] ?? ''),
                    'village' => mb_trim($data[6] ?? ''),
                    'telephone' => mb_trim($data[7] ?? ''),
                    'email' => mb_trim($data[8] ?? ''),
                    'school' => mb_trim($data[9] ?? ''),
                    'study_option' => mb_trim($data[10] ?? ''),
                    'entrance_year' => mb_trim($data[11] ?? ''),
                ]);
